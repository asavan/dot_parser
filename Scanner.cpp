...
namespace DOT {
	...
	static struct tagKeywords {
		...
		{"strict", tkStrict}, {"digraph", tkDigraph}, {"graph", tkGraph},
		{"node", tkNode},  {"edge", tkEdge},  {"subgraph", tkSubgraph}
	};
...
StartOver:
	...
	switch(chr) {
	case '{': return (cur_ = tkLeftBrace);
	case '}': return (cur_ = tkRightBrace);
	case ';': return (cur_ = tkSemicolon);
	case '=': return (cur_ = tkEqual);
	case '[': return (cur_ = tkLeftBracket);
	case ']': return (cur_ = tkRightBracket);
	case ',': return (cur_ = tkComma);
	case ':': return (cur_ = tkColon);
	case '(': return (cur_ = tkLeftParenthesis);
	case ')': return (cur_ = tkRightParenthesis);
	case '@': return (cur_ = tkAtSign);
	case '-':
		{
			is_.get(chr);
			if(!is_) {
